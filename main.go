...
func main() {
	...
	plusMinus := widget.NewButton("+/-", func() {
		if !strings.Contains(message, "=") {
			if symbol == "" {
				if val1 != "" {
					firstValue, _ := strconv.ParseFloat(val1, 32)
	...
	dis := container.New(layout.NewGridLayout(1), display)
	inp := container.New(layout.NewGridLayout(1), input)
	row1 := container.New(layout.NewGridLayout(3), modules, clear, back)
	row2 := container.New(layout.NewGridLayout(4), reciprocal, squre, squareRoot, divide)
	row3 := container.New(layout.NewGridLayout(4), seven, eight, nine, multipy)
	row4 := container.New(layout.NewGridLayout(4), four, five, six, subtract)
	row5 := container.New(layout.NewGridLayout(4), one, two, three, add)
	row6 := container.New(layout.NewGridLayout(4), plusMinus, zero, dot, equal)

	mainContainer := container.New(layout.NewGridLayout(1), dis, inp, row1, row2, row3, row4, row5, row6)

	w.SetContent(container.New(layout.NewGridLayoutWithRows(1), mainContainer))
	w.Resize(fyne.NewSize(320, 470))
	w.CenterOnScreen()
	w.ShowAndRun()
}
